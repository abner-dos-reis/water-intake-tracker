...
function App() {
  ...
  // instance id for cross-tab communication
  ...
  const queueSystemNotification = async (title = 'Time to drink water', message = 'Recommendation: 300ml') => {
    ...
    try {
      ...
      try {
        ...
        // include the app icon path (public/water.png) so host-notifier can display it
  ...
  // --- Inter-instance synchronization (BroadcastChannel + storage fallback) ---
  // We'll broadcast changes to intake and waterTarget so multiple open windows stay in sync.
  useEffect(() => {
    const channelName = 'water-sync-v1';
    let bc = null;
    try {
    ...
    const storageKey = 'water-sync-state-v1';

    const publishState = (state) => {
      const payload = { ...state, ts: Date.now(), inst: instanceIdRef.current };
      try {
    ...
    const onMessage = (msg) => {
      try {
        ...
        if (!data || data.inst === instanceIdRef.current) return; // ignore self
        // Only apply if newer
        ...
        if (data.ts && data.ts > (window.__water_last_ts__ || 0)) {
          ...
          if (typeof data.waterTarget === 'number') {
            isUserChange.current = false; // remote change, not local user edit
    ...
    // storage event fallback for tabs without BroadcastChannel
    const onStorage = (ev) => {
      try {
    ...
    // Publish initial state so others can pick it up
    publishState({ intake, waterTarget });
  ...
  // Whenever intake or waterTarget changes, publish state to other instances
  useEffect(() => {
    try {
      const storageKey = 'water-sync-state-v1';
      const payload = { intake, waterTarget, ts: Date.now(), inst: instanceIdRef.current };
      try { if ('BroadcastChannel' in window) { const bc = new BroadcastChannel('water-sync-v1'); bc.postMessage(payload); bc.close(); } } catch (e) {}
